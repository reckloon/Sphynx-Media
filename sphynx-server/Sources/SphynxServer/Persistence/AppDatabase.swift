...
struct AppDatabase: Sendable {
    ...
    private static var migrator: DatabaseMigrator {
        ...
        migrator.registerMigration("m27_item_placeholder_blurhash") { db in
            // A BlurHash string for the item's poster placeholder, generated by the
            // low-res-images extension when its mode is `blurhash`. Cached here so
            // serving never re-fetches/decodes the image; absent until generated, in
            // which case serving falls back to the URL placeholder. Sits alongside
            // `placeholderURL` (the always-derived tiny-image form).
            try db.alter(table: "item") { t in
                t.add(column: "placeholderBlurHash", .text)
            }
        }
